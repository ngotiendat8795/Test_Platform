    ...
    if 'username' in session:
        ...
        if request.method == "GET":
            ...
            time_accessed = reading_answer.find(
                {"USER": "ngotiendat8795", "TEST_ID": Test_ID, "Section": "R"}).count()
            form = request.form
            answer_sheet = {
...
def listening_test(id1, id2):
    if 'username' in session:
        if request.method == "GET":
            ...
            Test_ID = "IC"+str(id1)+"_T"+str(id2)
            time_accessed = reading_answer.find(
                {"USER": "ngotiendat8795", "TEST_ID": Test_ID, "Section": "L"}).count()
            form = request.form
            answer_sheet = {
                "USER": session["username"],
                "TEST_ID": Test_ID,
...
def writing_template(id1, id2):
    if 'username' in session:
        if request.method == "GET":
            ...
            return render_template('Writing_Test.html', Test_Content=Test_Content, session = session)
        elif request.method == "POST":
            Test_ID = "IC"+str(id1)+"_T"+str(id2)
            time_accessed = reading_answer.find(
                {"USER": "ngotiendat8795", "TEST_ID": Test_ID, "Section": "W"}).count()
            form = request.form
            answer_sheet = {
                "USER": session["username"],
                "TEST_ID": Test_ID,
...
def result(id1, id2, id3):
    Test_ID = "IC"+str(id1)+"_T"+str(id2)
    reading_history = reading_answer.find(
        {"USER": session['username'], "TEST_ID": Test_ID, "Section": "R"})
    time_accessed_r = reading_history.count()
    if time_accessed_r >= 1:
        latest_result_r = reading_history[time_accessed_r-1]

    listening_history = reading_answer.find(
        {"USER": session['username'], "TEST_ID": Test_ID, "Section": "L"})
    time_accessed_l = listening_history.count()
    if time_accessed_l >= 1:
        latest_result_l = listening_history[time_accessed_l-1]

    writing_history = reading_answer.find(
        {"USER": session['username'], "TEST_ID": Test_ID, "Section": "W"})
